...
if pooling_number==27:
    positive_samples_number=positive_slices-2
    all_samples=[]
    ...
    for samples in unique_samples:
        ...
        if count >= 3:
            positive_samples_list.append(str(samples))
            #print()
            #print('Sample ID '+str(samples)+' is positive')

    if len(positive_samples_list) > positive_samples_number:
        print('This is a case of degenerate cases, that is ambiguous results. Individual tests should be carried out for the potentially positive samples')
        print('Potentially positive samples are : ')
        for x in positive_samples_list:
            print('Sample ID '+x)
    else:
    ...
    positive_samples_number=positive_slices-3
    all_samples=[]
    positive_samples_list=[]
    for i in range(0,positive_slices):
